...
std::string RootHandler::HandleRequest(userver::server::http::HttpRequest& request,
    ...
    PrepareJson(request);
    userver::formats::json::ValueBuilder builder;
    builder["message"] = "Hotel Booking userver Event-Driven API with PostgreSQL and RabbitMQ is running";
    return userver::formats::json::ToString(builder.ExtractValue());
}
...
RegisterUserHandler::RegisterUserHandler(const userver::components::ComponentConfig& config,
    ...
    : HttpHandlerBase(config, context),
      ...
      events_(context.FindComponent<hotel_booking_hw06::EventPublisher>()) {}
...
std::string RegisterUserHandler::HandleRequest(userver::server::http::HttpRequest& request,
    ...
    try {
        ...
        const auto response = UserJson(selected.Front());
        ...
        events_.Publish("UserCreated", "User", user_id, "user.created", payload.ExtractValue());

        request.SetResponseStatus(userver::server::http::HttpStatus::kCreated);
        return SingleObject(response);
    } catch (const std::exception&) {
        return Fail(request, userver::server::http::HttpStatus::kBadRequest, "Invalid user payload");
    }
}
...
std::string CreateHotelHandler::HandleRequest(userver::server::http::HttpRequest& request,
    ...
    try {
        ...
        const auto response = HotelJson(selected.Front());
        ...
        payload["stars"] = stars;
        payload["rooms_total"] = rooms_total;
        payload["created_by"] = std::stoll(session->user_id);
        events_.Publish("HotelCreated", "Hotel", hotel_id, "hotel.created", payload.ExtractValue());
...
std::string CreateBookingHandler::HandleRequest(userver::server::http::HttpRequest& request,
    ...
    try {
        ...
        const auto user_id = std::stoll(session->user_id);
        const auto created = pg_->Execute(userver::storages::postgres::ClusterHostType::kMaster,
                                          R"(
        ...
        const auto response = BookingJson(selected.Front());
        ...
        events_.Publish("BookingCreated", "Booking", booking_id, "booking.created", payload.ExtractValue());
...
std::string CancelBookingHandler::HandleRequest(userver::server::http::HttpRequest& request,
    ...
    const auto current = pg_->Execute(userver::storages::postgres::ClusterHostType::kSlave,
                                      "SELECT user_id, hotel_id, status::text AS status FROM bookings WHERE id = $1",
                                      *booking_id);
    if (current.IsEmpty()) {
    ...
    const auto user_id = current_row["user_id"].As<std::int64_t>();
    const auto hotel_id = current_row["hotel_id"].As<std::int64_t>();
    if (session->user_id != std::to_string(user_id)) {
    ...
    events_.Publish("BookingCancelled", "Booking", *booking_id, "booking.cancelled", payload.ExtractValue());
