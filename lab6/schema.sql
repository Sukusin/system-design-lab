...
CREATE INDEX IF NOT EXISTS idx_event_log_type_time
    ON event_log(event_type, processed_at DESC);
...
CREATE TABLE IF NOT EXISTS booking_read_model (
    ...
    last_event_id TEXT NOT NULL REFERENCES event_log(event_id),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_booking_read_model_user
    ON booking_read_model(user_id, updated_at DESC);
